...
**Генераторы** - функция. Похожая на интератор, но он не требует всю коллекцию, может работать с новыми данными в коллекции и хранит только состояние, в котором находится. Интератор хранит всю коллекцию.
**Пример**
```Python 
def my_generator():
    yield 1
